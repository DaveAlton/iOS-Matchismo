...
-(NSString *)contents{
    return [NSString stringWithFormat:@"%@:%@:%@:%ld", self.symbol, self.color, self.shading, (unsigned long)self.number];
}
    ...
    if([otherCards count] == self.numberOfMatchingCards -1){
        ...
        for (id otherCard in otherCards){
            if([otherCard isKindOfClass:[SetCard class]]){
                ...
                if(([colors count] == 1 || [colors count] == self.numberOfMatchingCards) && ([symbols count] == 1 || [symbols count] == self.numberOfMatchingCards) &&
